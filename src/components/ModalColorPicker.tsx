...
export function ModalColorPicker({
  ...
  const [selectedColor, setSelectedColor] = useState(() => {
    return currentColor.startsWith('#') ? currentColor : hslToHex(currentColor);
  });

  const isColorTooSimilar = takenColors
    .filter((c) => c.toLowerCase() !== currentColor.toLowerCase())
    .some((c) => colorDistance(c, selectedColor) < 25);

  return (
    selectedColor && (
      <div className="absolute inset-0 bg-white/40 flex items-center justify-center z-50">
        <div className="relative bg-white p-6 rounded-xl shadow-2xl w-[260px] animate-fade-in flex flex-col items-center gap-4">
          <button
            onClick={onClose}
          ...
          {isColorTooSimilar && (
            <div className="text-sm text-red-600 text-center">
              Цвет слишком похож на уже выбранный другим игроком
            </div>
          )}
